...
const WorldDynamic = dynamic(() => import("../ui/globe").then((m) => m.World), {
  ssr: false,
});
...
export default function TravelGlobe() {
  ...
  const flyToPlace = useCallback(
    (place) => {
      ...
      if (!globeReady || !worldRef.current) {
        console.warn(
          "[TravelGlobe] Globe not ready, cannot fly. Ready:",
          globeReady,
          "Ref:",
          !!worldRef.current,
        );
      ...
      if (typeof worldRef.current.flyTo !== "function") {
        console.error(
          "[TravelGlobe] flyTo method not available on worldRef.current",
        );
  ...
  const handleGlobeReady = useCallback(() => {
    ...
    if (!worldRef.current) {
      console.warn(
        "[TravelGlobe] worldRef.current is null in handleGlobeReady",
      );
      return;
    }
    ...
    try {
      if (typeof worldRef.current.flyTo === "function") {
        ...
        console.warn(
          "[TravelGlobe] flyTo method not available during initialization",
        );
      }
    } catch (error) {
        ...
        {myPlaces.map((place, idx) => (
          ...
          >
            ...
            <img
              ...
              onError={(e) => {
                console.warn(
                  `[TravelGlobe] Failed to load image: ${place.image}`,
                );
